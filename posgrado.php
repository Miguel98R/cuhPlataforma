<?php include "components/head.php";?>
<body>
  <?php include "components/nav.php";?>
  <!-- Start your project here-->  
  
<?php include "components/header.php";?>

<div class="container-fluid">


  <div class="text-center" style="  background-image: url(img/fondo.jpg);">

    

     <!-- Card -->


     <div class="view overlay px-2 py-2">
    <img style="width: 100%; height: 600px;"class="card-img-top" src="img/posgrado3.jpg  "alt="Card image cap">
    <a href="#!">
      <div class="mask rgba-white-slight">
       
      </div>
    </a>
  </div>


    <div class="row">
        <div class="col-md-8">
            <section class="py-2 px-2">



  <div class="container text-center py-4">
      <div class="row">
          <div class="col-md-4 py-2">
              <div class="card  danger-color-dark" style="width: 18rem;">
 <a href=""></a> 
  <div class="card-body red darken-2">
 
    <button class="btn text-white font-weight-bold">Finanzas</button>
  </div>
</div>
              

          </div>
          <div class="col-md-4 py-2">

                     <div class="card  red lighten-3" style="width: 18rem;">
 <a href=""></a> 
  <div class="card-body pink lighten-1">
 
 <button class="btn text-white font-weight-bold">Administración de negocios</button>
  </div>
</div>
        

          </div>
          <div class="col-md-4 py-2">

                     <div class="card  green lighten-2" style="width: 18rem;">
 <a href=""></a> 
  <div class="card-body green accent-3">
 
   <button class="btn text-white font-weight-bold">Derecho civil</button>
  </div>
</div>
        

          </div>

      </div>
        <div class="row py-3">
            <div class="col-md-4 py-2">

                         <div class="card   cyan darken-3" style="width: 18rem;">
 <a href=""></a> 
  <div class="card-body cyan darken-2">
 
     <button class="btn  text-white font-weight-bold">Formación docente</button>
  </div>
</div>
        

          </div>
         <div class="col-md-4 py-2">

                     <div class="card  lime darken-4" style="width: 18rem;">
 <a href=""></a> 
  <div class="card-body lime darken-3">
 
    <button class="btn text-white font-weight-bold">Estrategias de intervención en problemas de aprendizaje</button>
  </div>
</div>
        

          </div>
         <div class="col-md-4 py-2">

                     <div class="card  blue-grey darken-4" style="width: 18rem;">
 <a href=""></a> 
  <div class="card-body blue-grey darken-3">
 
    <button class="btn text-white font-weight-bold">Derecho procesal penal acusatorio</button>
  </div>
</div>
        

          </div>

      </div>

  </div>
  
      

    </section>
        </div>
        <div class="col-md-4">
            <section class="py-5">
        <div class="container">
           
                    <div class="alert alert-primary" role="alert">
                   <p> <strong class="font-weight-bold">Tipo de Ciclo:</strong> Cuatrimestral</p>
                        <p><strong  class="font-weight-bold">Duración del Ciclo Sabatino:</strong> 1 Año 4 meses</p>
                    
                    </div>
                </div>
           
     
     
    </section>

      
        </div>
    </div>
    

    

    
   

   

  </div>

</div>
<?php include "components/footer.php";?>






  <!-- End your project here-->

  <!-- jQuery -->
  <script type="text/javascript" src="js/jquery.min.js"></script>
  <!-- Bootstrap tooltips -->
  <script type="text/javascript" src="js/popper.min.js"></script>
  <!-- Bootstrap core JavaScript -->
  <script type="text/javascript" src="js/bootstrap.min.js"></script>
  <!-- MDB core JavaScript -->
  <script type="text/javascript" src="js/mdb.min.js"></script>
  <!-- Your custom scripts (optional) -->
  <script type="text/javascript"></script>

</body>
</html>
